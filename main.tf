...
resource "aws_iam_role_policy" "ssm_execution" {
  count  = length(jsondecode(data.aws_iam_policy_document.task_ecs_ssm_policy.json).Resources) > 0 ? 1 : 0
...
resource "aws_iam_role_policy" "kms_execution" {
  count  = length(jsondecode(data.aws_iam_policy_document.task_ecs_kms_policy.json).Resources) > 0 ? 1 : 0
